...
- **UI-07 (Ações Contextuais sobre Menus Globais e Progressive Profiling):**  
  Para evitar poluição visual e excesso de cliques, os módulos devem priorizar a abertura de ações diretamente via interação com os painéis e cartões (Dashboards/Kanbans - modais contextuais) ao invés de listas isoladas em menus laterais. Além disso, a captura de dados de pacientes deve seguir o conceito de **Progressive Profiling**, solicitando informações (como telefones ou detalhes de leito) estruturalmente nas etapas em que são realmente devidas, enxugando o formulário de Cadastro inicial obrigatório.
